...
Клиент отправляет запрос на корпоративную почту.

Почтовый сервис передаёт письмо в Backend.

Backend направляет данные в ML-модуль.

ML-модуль обрабатывает письмо и возвращает результат.

Backend формирует итоговую структуру ответа.

Результат передаётся во Frontend.

Оператор поддержки работает с данными через веб-интерфейс.
...
Дополнительный риск связан с возможностью некорректного переобучения модели. Минимизируется за счёт ограничения доступа к обновлению базы знаний и контроля со стороны ответственных специалистов.
